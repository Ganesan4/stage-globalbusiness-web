...
export class MyaccountComponent implements OnInit {
  ...
  referralLink: string = '';
  referredBy: string = '';
  showReferralCopyMessage: boolean = false;
  showLinkCopyMessage: boolean = false;

  constructor(private fb: FormBuilder, private paymentService: PaymentService, private referralService: ReferralService, private renderer: Renderer2,private http: HttpClient, private web: WebService) {
  ...
  async myprofile(){
    try{
      ...
      if(res.status){
        this.myprofiledata = res.message
        this.lastUpdatedOn = this.myprofiledata.last_updated_on
        console.log("this.myprofiledata",this.myprofiledata);

        if (!this.myprofiledata.logo || this.myprofiledata.logo === 'null' || this.myprofiledata.logo.endsWith('/undefined')) {
          this.logoUrl = 'assets/img/dummy.jpg';
          this.selectedFileName = 'No file chosen';
        } else {
          this.logoUrl = this.myprofiledata.logo;
          this.selectedFileName = this.myprofiledata.logo;
        }
        ...
        this.registerForm.patchValue({
          ...
          // lastUpdatedOn: this.myprofiledata.last_updated_on
        });
        console.log("this.registerForm",this.logoUrl)

        // Load and display user's referral code
        if (this.myprofiledata.referral_code) {
          this.userReferralCode = this.myprofiledata.referral_code;
          this.referralLink = this.referralService.getReferralLink(this.userReferralCode);
        }

        // Display who referred the user (optional)
        if (this.myprofiledata.referred_by) {
          this.referredBy = this.myprofiledata.referred_by;
        }

        // After patching the form, fetch states if country is set
        if (this.myprofiledata.country) {
          const selectedCountry = this.countries.find(country => country.iso3 === this.myprofiledata.country);
          if (selectedCountry) {
    ...
    this.http.get<{ data: { iso2: string; iso3: string; name: string; states: any[] }[], error: boolean, msg: string }>(`${this.apiUrl}getCountriesStates`).subscribe(
      (response) => {
        ...
        if (!response.error && response.data) {
          ...
          if (this.myprofiledata && this.myprofiledata.country) {
            const selectedCountry = this.countries.find(country => country.iso3 === this.myprofiledata.country);
            if (selectedCountry) {
              this.fetchStates(selectedCountry.name);
            }
        ...
        if (!response.error && response.data?.length) {
          ...
          if (this.myprofiledata?.state) {
            const matchingState = this.states.find(
              s => s.state_code === this.myprofiledata.state
            );
  ...
  copyReferralCode(): void {
    this.referralService.copyToClipboard(this.userReferralCode).then(() => {
      this.showReferralCopyMessage = true;
      setTimeout(() => {
        this.showReferralCopyMessage = false;
      }, 2000);
    }).catch((error) => {
  ...
  copyReferralLink(): void {
    this.referralService.copyToClipboard(this.referralLink).then(() => {
      this.showLinkCopyMessage = true;
      setTimeout(() => {
        this.showLinkCopyMessage = false;
      }, 2000);
    }).catch((error) => {
